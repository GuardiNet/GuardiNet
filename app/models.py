...
class Absence(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    # made nullable to allow manual absence without strictly needing a schedule event initially, or we can keep it strict. Actually I will keep it strict but add course_id for generic absence tracking if schedule isn't there
...
class TicketMessage(db.Model):
    ...
    ticket = db.relationship('SupportTicket', backref=db.backref(
        'ticket_comments', cascade='all, delete-orphan', lazy=True))
